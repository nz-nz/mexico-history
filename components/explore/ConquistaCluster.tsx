...
import { ConquistaProfile } from '../../data/conquistaColoniaExplorar';
import { WallTooltip } from './WallTooltip';
import { AchievementBadge } from './AchievementBadge';
import { ImageModal } from './ImageModal';
import { User, Calendar, Sword, Crown, Building, Scroll, Quote } from 'lucide-react';

interface ConquistaClusterProps {
  item: ConquistaProfile;
}

const getCategoryLabel = (category: ConquistaProfile['category']) => {
  switch (category) {
    case 'conquista': return 'Conquista';
    case 'caida': return 'Caída';
    case 'nueva_espana': return 'Nueva España';
    case 'sociedad': return 'Sociedad';
    case 'cadiz': return 'Cádiz';
  }
};

const getCategoryColor = (category: ConquistaProfile['category']) => {
  switch (category) {
    case 'conquista': return 'bg-red-700';
    case 'caida': return 'bg-amber-700';
    case 'nueva_espana': return 'bg-yellow-600';
    case 'sociedad': return 'bg-purple-700';
    case 'cadiz': return 'bg-blue-700';
  }
};

const getCategoryIcon = (category: ConquistaProfile['category']) => {
  switch (category) {
    case 'conquista': return <Sword className="w-3 h-3" />;
    case 'caida': return <Crown className="w-3 h-3" />;
    case 'nueva_espana': return <Building className="w-3 h-3" />;
    case 'sociedad': return <User className="w-3 h-3" />;
    case 'cadiz': return <Scroll className="w-3 h-3" />;
  }
};

export const ConquistaCluster: React.FC<ConquistaClusterProps> = ({ item }) => {
          ...
          <div className="relative group cursor-pointer transition-all duration-200 hover:scale-[1.03] hover:shadow-xl">
            <div className={`${portraitSize} rounded-xl overflow-hidden shadow-lg border-2 border-white/30 dark:border-gray-600/50`}>
              {!imgError ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-amber-200 to-yellow-200 dark:from-amber-900/50 dark:to-yellow-900/50 flex items-center justify-center">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 rounded-b-xl">
              ...
              {!item.subtitle && item.nickname && (
                <div className="text-yellow-300 text-[10px] md:text-xs">
                  "{item.nickname}"
                </div>
              )}
...
export default ConquistaCluster;
