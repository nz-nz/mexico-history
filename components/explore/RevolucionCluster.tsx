...
import { RevolucionProfile } from '../../data/revolucionExplorar';
import { WallTooltip } from './WallTooltip';
import { AchievementBadge } from './AchievementBadge';
import { ImageModal } from './ImageModal';
import { User, Calendar, Scroll, Flag, BookOpen, Quote } from 'lucide-react';

interface RevolucionClusterProps {
  item: RevolucionProfile;
}

const getCategoryLabel = (category: RevolucionProfile['category']) => {
  switch (category) {
...
const getCategoryColor = (category: RevolucionProfile['category']) => {
  switch (category) {
    case 'porfiriato': return 'bg-amber-700';
    case 'lider': return 'bg-red-700';
    case 'plan': return 'bg-blue-700';
    case 'evento': return 'bg-purple-700';
    case 'constitucion': return 'bg-green-700';
  }
};

const getCategoryIcon = (category: RevolucionProfile['category']) => {
  switch (category) {
    case 'porfiriato': return <User className="w-3 h-3" />;
    case 'lider': return <Flag className="w-3 h-3" />;
    case 'plan': return <Scroll className="w-3 h-3" />;
    case 'evento': return <Calendar className="w-3 h-3" />;
    case 'constitucion': return <BookOpen className="w-3 h-3" />;
  }
};

export const RevolucionCluster: React.FC<RevolucionClusterProps> = ({ item }) => {
  ...
  const tooltipContent = (
    <div>
      ...
      {item.subtitle && (
        <div className="text-xs text-red-300">{item.subtitle}</div>
      )}
      {item.nickname && !item.subtitle && (
        <div className="text-xs text-red-300">"{item.nickname}"</div>
      )}
      {item.date && (
      ...
      <div className="text-sm mt-1 text-red-200">{item.description}</div>

      {/* Famous Quote */}
      {item.famousQuote && (
        <div className="mt-2 p-2 bg-red-900/30 rounded-lg border-l-2 border-red-500">
          <div className="flex items-start gap-1">
            <Quote className="w-3 h-3 text-red-400 mt-0.5 flex-shrink-0" />
            <div>
              <div className="text-xs text-red-200 italic">"{item.famousQuote.text}"</div>
          ...
          <div className="space-y-1">
            {item.facts.map((fact, i) => (
              <div key={i} className="text-xs">
                <span className="text-red-300">{fact.label}:</span>{' '}
  ...
  return (
    <div className="flex flex-col gap-3">
          ...
          <div className="relative group cursor-pointer transition-all duration-200 hover:scale-[1.03] hover:shadow-xl">
            <div className={`${portraitSize} rounded-xl overflow-hidden shadow-lg border-2 border-white/30 dark:border-gray-600/50`}>
              {!imgError ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-red-200 to-amber-200 dark:from-red-900/50 dark:to-amber-900/50 flex items-center justify-center">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 rounded-b-xl">
              ...
              {item.subtitle && (
                <div className="text-red-300 text-[10px] md:text-xs">
                  {item.subtitle}
                </div>
              )}
      ...
      {/* Famous Quote Badge */}
      {item.famousQuote && (
        <div className="flex items-center gap-1 px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200 rounded-lg text-[10px]">
      ...
      {item.examFacts.length > 0 && !item.famousQuote && (
        <div className="flex flex-wrap gap-1">
          {item.examFacts.slice(0, 2).map((fact, index) => (
            <span
              key={index}
              className="text-[10px] px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200 rounded-full"
            >
...
export default RevolucionCluster;
